...
class BodyPart {
    ...
    isUrlButtonTypeDisabled(index) {
        ...
        if (!buttons)
            return false;
        return buttons.filter((b, i) => i !== index && b.type === 'URL').length > 1;
    }
...
class FooterPart {
    ...
    get isInvalid() {
        ...
        if (buttons?.buttons.find(x => {
            ...
            if (x.type === 'URL')
                return x.text?.length === 0 || x.url?.trim().length === 0;
            ...
            return true;
        const urlButtons = (buttons?.buttons ?? []).filter(x => x.type === 'URL');
        if (urlButtons.length > 2)
            return true;
        const urlButtonTexts = urlButtons.map(b => b.text).filter(e => e);
        if (new Set(urlButtonTexts).size !== urlButtonTexts.length)
            return true;
        return false;
    }
